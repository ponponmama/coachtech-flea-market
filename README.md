...
以下の 3 つのユーザーデータが作成されます：

1. **test@01.com** - CO01~CO05 の商品データを出品したユーザー

   - CO01: 腕時計（15,000 円）
   - CO02: HDD（5,000 円）
   - CO03: 玉ねぎ 3 束（300 円）
   - CO04: 革靴（4,000 円）
   - CO05: ノート PC（45,000 円）

2. **test@02.com** - CO06~CO10 の商品データを出品したユーザー

   - CO06: マイク（8,000 円）
   - CO07: ショルダーバッグ（3,500 円）
   - CO08: タンブラー（500 円）
   - CO09: コーヒーミル（4,000 円）
   - CO10: メイクセット（2,500 円）

3. **test@03.com** - 何も紐づけられていないユーザー
   - 商品の出品、購入、取引メッセージなど、何も紐づけられていない状態のユーザー

### 商品データ（CO01~CO10）

要件に基づいて、以下の商品データが作成されます：

**CO01~CO05（test@01.comが出品）:**

- CO01: 腕時計 - 15,000 円 - 良好
- CO02: HDD - 5,000 円 - 目立った傷や汚れなし
- CO03: 玉ねぎ 3 束 - 300 円 - やや傷や汚れあり
- CO04: 革靴 - 4,000 円 - 状態が悪い
- CO05: ノート PC - 45,000 円 - 良好

**CO06~CO10（test@02.comが出品）:**

- CO06: マイク - 8,000 円 - 目立った傷や汚れなし
- CO07: ショルダーバッグ - 3,500 円 - やや傷や汚れあり
- CO08: タンブラー - 500 円 - 状態が悪い
- CO09: コーヒーミル - 4,000 円 - 良好
- CO10: メイクセット - 2,500 円 - 目立った傷や汚れなし

> **注意**: これらの商品は `ItemSeeder` で要件に基づいて作成されます。デフォルトでは取引中（buyer_id が null）の状態で作成されます。
...
> **補足**: アプリケーションでは、取引中の商品は「buyer_id が null の商品」だけでなく、「buyer_id が設定されているが評価が完了していない商品（両方が評価するまで）」も含まれます。これにより、決済処理が完了した後でも、評価が完了するまで取引中の商品として扱われます。
...
- **機能テスト**
  ...
  - `EmailVerificationTest.php` - メール認証機能テスト（7 項目）
    ...
    - 認証メール再送信機能
    - 未認証ユーザーの認証通知画面アクセス制限
    - 認証済みユーザーの認証通知画面アクセス制限
    - 無効な認証 URL の処理
  ...
  - `TransactionChatTest.php` - 取引チャット確認機能テスト（9 項目）

    - マイページから取引中の商品を確認することができる（ID: FN001）
    - 未読メッセージ数をマイページで確認することができる（ID: FN001）
    - マイページから取引チャット画面に遷移することができる（ID: FN002）
    - サイドバーから他の取引チャット画面に遷移することができる（ID: FN003）
    - 取引中の商品の並び順は新規メッセージが来た順に表示する（ID: FN004）
    - 新規通知が来た商品は、取引中の各商品の左上に通知マークを表示する（ID: FN005）
    - 通知マークから何件メッセージが来ているかが確認できる（ID: FN005）
    - 購入した商品（buyer_id が設定されている）も取引中の商品に含まれる（ID: FN001）
    - 購入した商品で両方が評価した場合は取引中の商品から除外される（ID: FN001）

  - `RatingAverageTest.php` - 評価平均確認機能テスト（4 項目）

    - 他ユーザーからの取引評価の平均をプロフィール画面にて表示する（ID: FN005）
    - まだ評価がないユーザーの場合は評価は表示しない
    - 評価数の平均値に小数がある場合、その値は四捨五入する

  - `TransactionMessageTest.php` - 取引チャット投稿機能テスト（9 項目）

    - 取引チャットで本文を送信することができる（ID: FN006）
    - 取引チャットで画像を送信することができる（ID: FN006）
    - 取引チャットで本文と画像の両方を送信することができる（ID: FN006）
    - 本文が未入力の場合、バリデーションメッセージが表示される（ID: FN008）
    - 本文が 401 文字以上の場合、バリデーションメッセージが表示される（ID: FN008）
    - 画像が.png または.jpeg 形式以外の場合、バリデーションメッセージが表示される（ID: FN008）
    - .png 形式の画像を送信することができる（ID: FN007）
    - .jpeg 形式の画像を送信することができる（ID: FN007）
    - チャットを入力した状態で他の画面に遷移しても、入力情報を保持できる（ID: FN009）

  - `TransactionMessageEditTest.php` - メッセージ編集・削除機能テスト（6 項目）

    - 投稿済みのメッセージを編集することができる（ID: FN010）
    - 投稿済みのメッセージの画像を編集することができる（ID: FN010）
    - 他のユーザーのメッセージを編集できない（ID: FN010）
    - 投稿済みのメッセージを削除することができる（ID: FN011）
    - 画像付きメッセージを削除すると画像も削除される（ID: FN011）
    - 他のユーザーのメッセージを削除できない（ID: FN011）

  - `TransactionRatingTest.php` - 取引評価機能テスト（8 項目）

    - 取引を完了ボタンをクリックすると取引完了モーダルからユーザーの評価をすることができる（購入者）（ID: FN012）
    - 商品の購入者が取引を完了した後に、取引チャット画面を開くと取引完了モーダルからユーザーの評価をすることができる（出品者）（ID: FN013）
    - 評価を送信した後は、商品一覧画面に遷移する（ID: FN014）
    - 評価が選択されていない場合、バリデーションメッセージが表示される
    - 決済処理が完了していない場合、評価できない
    - 購入者または出品者以外は評価できない
    - 決済処理が完了した後でも、購入者がまだ評価していない場合は取引完了ボタンが表示される（ID: FN012）
    - 出品者は取引完了ボタンが表示されない（ID: FN012）

  - `TransactionEmailNotificationTest.php` - メール通知機能テスト（4 項目）

    - 商品購入者が取引を完了すると、商品出品者宛に自動で通知メールが送信される（ID: FN016）
    - 既に取引が完了している場合、メールは再送信されない（ID: FN016）
    - 購入者以外が取引完了を試みた場合、メールは送信されない（ID: FN016）
    - メール通知の内容が正しい（ID: FN015, FN016）
